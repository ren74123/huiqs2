...
-- Add archived status to travel_packages status check constraint
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'travel_packages_status_check'
    AND table_name = 'travel_packages'
  ) THEN
    ALTER TABLE travel_packages DROP CONSTRAINT travel_packages_status_check;
  END IF;
END $$;

-- Add new constraint with archived status
ALTER TABLE travel_packages
ADD CONSTRAINT travel_packages_status_check
