...
DROP POLICY IF EXISTS "admin_message_logs_policy_v7" ON message_logs;
DROP POLICY IF EXISTS "admin_message_logs_policy_v8" ON message_logs;
DROP POLICY IF EXISTS "agent_message_logs_policy_v7" ON message_logs;
DROP POLICY IF EXISTS "agent_message_logs_policy_v8" ON message_logs;
DROP POLICY IF EXISTS "user_message_logs_policy_v7" ON message_logs;
...
-- Create new function
CREATE OR REPLACE FUNCTION sync_order_status_v8()
RETURNS trigger AS $$
BEGIN
...
-- Create new trigger
CREATE TRIGGER sync_order_status_trigger_v8
  BEFORE UPDATE OF status ON orders
  FOR EACH ROW
  EXECUTE FUNCTION sync_order_status_v8();