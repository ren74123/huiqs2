...
-- Create new function
CREATE OR REPLACE FUNCTION sync_order_status_v6()
RETURNS trigger AS $$
BEGIN
...
-- Create new trigger
CREATE TRIGGER sync_order_status_trigger_v6
  BEFORE UPDATE OF status ON orders
  FOR EACH ROW
  EXECUTE FUNCTION sync_order_status_v6();
...
DROP POLICY IF EXISTS "admin_message_logs_policy_v6" ON message_logs;
DROP POLICY IF EXISTS "agent_message_logs_policy_v6" ON message_logs;
DROP POLICY IF EXISTS "user_message_logs_policy_v6" ON message_logs;

-- Enable RLS
ALTER TABLE message_logs ENABLE ROW LEVEL SECURITY;

-- Create policies with unique names
CREATE POLICY "admin_message_logs_policy_v8"
ON message_logs
FOR ALL
TO public
USING (
...
CREATE POLICY "agent_message_logs_policy_v8"
ON message_logs
FOR ALL
TO public
USING (
...
CREATE POLICY "user_message_logs_policy_v8"
ON message_logs
FOR SELECT
TO public
USING (
