...
import { ArrowLeft, Coins, Search, Calendar, AlertCircle } from 'lucide-react';
...
export function InfoFeeHistory() {
  ...
  function filterLogs() {
    ...
    if (searchTerm) {
      filtered = filtered.filter(log => 
        log.order?.travel_packages?.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">信息费记录</h1>
      </div>
                  ...
                  <tr key={log.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-gray-900">
                        {log.order?.travel_packages?.title || '未知套餐'}
                      </div>
                      <div className="text-sm text-gray-500">
                        订单号: {log.order_id.slice(0, 8)}
                      </div>
                    </td>
