...
export function Messages() {
  ...
  async function fetchMessages() {
    try {
      ...
      const { data: profile, error: profileError } = await request({
        url: `/api/profile/${user?.id}`,
        method: 'GET'
      });
      ...
      if (profile.user_role === 'user' || profile.user_role === 'agent') {
        query = request({
          url: `/api/messages?receiver_id=${user?.id}`,
          method: 'GET'
        });
      }
      ...
      if (profile.user_role === 'admin') {
        query = request({
          url: `/api/messages?receiver_id=${user?.id}&type=system&sender_id=${user?.id}`,
          method: 'GET'
        });
      }
      ...
      const messagesWithOrderInfo = await Promise.all((data || []).map(async (msg) => {
        ...
        if (msg.content.includes('订单') || msg.content.includes('套餐')) {
          ...
          if (orderMatch && orderMatch[1]) {
            ...
            const { data: packageData } = await request({
        ...
        if (msg.content.includes('企业团建')) {
          ...
          if (enterpriseMatch && enterpriseMatch[1]) {
            ...
            const { data: enterpriseData } = await request({
              url: `/api/enterprise/orders/${enterpriseId}`,
              method: 'GET'
            });
 ...
 const { data, error } = await request({
 url: `/api/message-logs?agent_id=${user.id}`,
 method: 'GET'
 });
