...
import { Package, MapPin, Calendar, Clock, AlertCircle, MessageSquare, UserCircle, Star, FileCheck, Gift } from 'lucide-react';
...
export function Orders() {
  ...
  const { total, fetchCredits } = useCreditStore();
  ...
  useEffect(() => {
    if (user) {
      ...
      fetchUserReviews();
      if (user.id) {
        console.log("Orders: 获取积分数据，用户ID:", user.id);
      ...
      if (data.length > 0) {
        ...
        for (const order of data) {
          ...
          const messageResponse = await request({
            url: `/api/messages/${order.id}`,
            method: 'GET'
          });

          if (messageResponse.data) {
            orderMessages[order.id] = messageResponse.data;
          }
        }
  ...
  const handleContractConfirm = async () => {
    ...
    try {
      ...
      await request({
        url: `/api/orders/${contractConfirmOrderId}/contract`,
        method: 'PATCH',
        data: {
  ...
  const filteredOrders = orders
    .filter(order => showRejected ? true : order.status !== 'rejected')
    .filter(Boolean); // 确保过滤掉任何null/undefined订单
    ...
    <div className="space-y-4 w-full">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center bg-[#FFF0F5] px-3 py-1 rounded-full">
            <Gift className="h-4 w-4 text-[#F52E6B] mr-1" />
            <span className="text-sm font-medium text-[#F52E6B]">{total}积分</span>
          </div>
        ...
        >
          <div className="flex flex-col">
            <div className="w-full h-48 bg-gray-100 flex items-center justify-center relative">
              {order.travel_packages?.image ? (
                <img 
                  src={order.travel_packages.image}
                  alt={order.travel_packages.title || '旅行套餐图片'}
            ...
            <div className="p-4">
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="font-semibold text-gray-900 min-h-[1.5em] break-words">
                    {order.travel_packages?.title || '未命名套餐'}
                  </h3>
