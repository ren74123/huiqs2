...
export function CreditSettings({ activeSettingsTab = 'credits', orderId = null, enterpriseOrderId = null }: CreditSettingsProps) {
  ...
  const handlePaymentSuccess = async () => {
    ...
    try {
      // In a real app, this would be called by a webhook after payment confirmation
      // For now, we'll simulate a successful purchase
      const { error } = await supabase.rpc('add_credits', {
        p_user_id: user?.id,
        p_amount: selectedPackage.credits,
        p_remark: `购买${selectedPackage.name}（${selectedPackage.credits}积分）`
      });
      ...
      setStatusMessage(`成功购买了 ${selectedPackage.credits} 积分！`);
